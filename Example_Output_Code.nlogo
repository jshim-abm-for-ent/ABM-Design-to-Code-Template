...
; Generated from the Example_Canvas_Filled_EN.md

globals [
  lambda alpha beta gamma
  reinforce-rate decay-rate
  avg-P network-density sys-performance
  policy-mode
]
...
to go
  ask founders [
    act                                   ; [4] Behavioral Rules
    interact                              ; [5] Interaction Mechanisms
  ]
  decay-ties
  update-embeddedness
  update-metrics
...
to act
  ; pick one linked owner (if any) and attempt acquisition
  if any? my-ties [
...
to apply-policy
  if policy-mode = 1 [
    ; networking boost: could scale link formation prob in setup/go (left as hook)
  ]
  if policy-mode = 2 [
